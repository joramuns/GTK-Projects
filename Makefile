...
gtk:
	gcc -fsanitize=address -g -c GUI/*.c calculator.c Validation/*.c Calculation/*.c `pkg-config --cflags --libs gtk4`
	gcc -fsanitize=address -g -o test *.o `pkg-config --libs gtk4`
	./test

gtk-lasan:
	gcc -fsanitize=leak -g -c GUI/*.c calculator.c Validation/*.c Calculation/*.c `pkg-config --cflags --libs gtk4`
	gcc -fsanitize=leak -g -o test *.o `pkg-config --libs gtk4`
	./test

gtk-noasan:
	gcc -g -c GUI/*.c calculator.c Validation/*.c Calculation/*.c `pkg-config --cflags --libs gtk4`

...
typedef struct credit_input {
  ...
  unsigned term; /**< Amount of monthes to be credited, different types of input
                    as years evaluates the number of monthes */
  double rate;   /**< Interest rate */
  unsigned type; /**< There are 2 different types of credit: 0 - for annuity, 1
                    - for differentiated */
} credit_input;
...
typedef struct credit_output {
  ...
  node *stack_of_payments; /**< Stores a node list with monthly payments for
                              credit, needs to be freed */
  double
      overpaid;  /**< Calculates the sum of overpayment due to interest rate */
} credit_output; /**< */

/**
 * @brief Main credit calculator function that heads in two ways depending on
 * type of credit. It also converts interest rate in credit_input to real number
 * and counts monthly interest rate
 *
...
/**
 * @brief A function to calculate needed data on loan conditions for

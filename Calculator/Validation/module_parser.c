  ...
  while (array_pos++ < len) {
    if (expression[array_pos] == 'X') {
      ...
      if (check_forward == TOK_NUM || check_forward == TOK_DOT ||
          check_forward == TOK_UNARY || check_forward == TOK_OPEN_BRACE) {
        ex_code = SURROUNDING_VARIABLE;
      }
    }
  }
    ...
    if (ISDOT) {
      *array_pos += 1;
      ...
      while (ISDIGIT) {
        parse_divider /= 10.0;
        last->value =
            (expression[*array_pos] - '0') * parse_divider + last->value;
        *array_pos += 1;
      }
...
int parse_long_operator(char *expression, size_t *array_pos) {
  ...
  if (len <= 2) {
    ...
    *array_pos += position_move;
    if (ex_code != CODE_MOD) {
      if (expression[*array_pos] != '(' ||
          expression[(*array_pos) + 1] == ')' ||
...
void handle_unary(const char *expression, size_t array_pos,
                  node *output_stack) {

...
void execute_deposit_func(const GtkButton *widget, gpointer data) {
  if (widget && data) {
    int ex_code = 0;
    ...
    GtkEntryBuffer *tax_rate_entry_buf =
        gtk_entry_get_buffer(entry->tax_rate_entry);
    char *tax_output_rate =
        (char *)gtk_entry_buffer_get_text(tax_rate_entry_buf);
    ...
    unsigned input_term =
        gtk_combo_box_get_active(GTK_COMBO_BOX(entry->term_cbt));
    if (input_term == 1) {
      cont.term *= 30;
    ...
    unsigned input_period =
        gtk_combo_box_get_active(GTK_COMBO_BOX(entry->type_payouts));
    if (input_period == 0) {
    ...
    for (unsigned i_am = 0; i_am < cont.term; i_am++)
      cont.account_movement[i_am] = 0;
    ...
    cont.capitalization =
        gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(entry->type_credit_cbt));
...
void add_withdrawal_func(const GtkButton *button, gpointer data_struct) {
  if (button) {
    ...
    if (*sum_char && *date_char) {
      ...
      if (ex_code == 0)
        ex_code = (strchr(date_char, '.') == NULL) ? 0 : WRONG_SYMBOLS;
      if (ex_code == 0) {
        GtkTreeIter iter;
...
void del_withdrawal_func(const GtkButton *button, gpointer data_struct) {
  if (button) {
    gboolean valid_iter;
    wd_cont *withdrawal_cont = data_struct;
    GtkTreeStore *store = withdrawal_cont->withdrawal_window;
...
void deposit_calc_window() {
  ...
  gtk_builder_add_from_resource(builder, "/org/gtk/Style/deposit-calc-o.ui",
                                NULL);
  ...
  GtkComboBoxText *tax_percentage_cbt =
      (GtkComboBoxText *)gtk_builder_get_object(builder, "tax_percents");
  ...
  GtkCheckButton *type_choice =
      (GtkCheckButton *)gtk_builder_get_object(builder, "type_choice");
  GtkComboBoxText *type_payouts =
      (GtkComboBoxText *)gtk_builder_get_object(builder, "type_payouts");
  gtk_combo_box_set_active(GTK_COMBO_BOX(type_payouts), 0);

  /* Withdrawal block */
  GtkButton *add_withdrawal =
      (GtkButton *)gtk_builder_get_object(builder, "withdrawal_plus");
  GtkButton *del_withdrawal =
  ...
  column = gtk_tree_view_column_new_with_attributes("Date", renderer, "text", 0,
                                                    NULL);
  /* Add the column to the view. */
  gtk_tree_view_append_column(GTK_TREE_VIEW(view_list), column);
  column = gtk_tree_view_column_new_with_attributes("Sum", renderer, "text", 1,
                                                    NULL);
  gtk_tree_view_append_column(GTK_TREE_VIEW(view_list), column);
  ...
  g_signal_connect(add_withdrawal, "clicked", G_CALLBACK(add_withdrawal_func),
                   withdrawal_cont);
  g_signal_connect(del_withdrawal, "clicked", G_CALLBACK(del_withdrawal_func),
                   withdrawal_cont);
  ...
  g_signal_connect(execute_button, "clicked", G_CALLBACK(execute_deposit_func),
                   one);

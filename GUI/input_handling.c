...
void get_result(const GtkButton *widget, gpointer data) {
  if (widget) {
    ...
    gboolean graph_status =
        gtk_toggle_button_get_active(current->toggle_button);
    ...
    if (ex_code == 0) {
      ...
      gtk_drawing_area_set_draw_func(GTK_DRAWING_AREA(current->area),
                                     graph_draw, output, NULL);

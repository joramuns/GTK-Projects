...
static const GLfloat sc_vertices_expected[] = {
  ...
  -4.211081, -0.779347, -3.616526, 1.0,
  2.107359,  3.588090,  -0.592267, 1.0,
  2.490416,  -0.544245, -2.935667, 1.0,
  -1.006018, 5.145445,  -3.847413, 1.0,
  -0.622962, 1.013109,  -6.190813, 1.0,
};
...
START_TEST(test_simple_cube_vertices_data)
{
  ...
  ck_assert_float_eq_tol(data[_i * 4 + 1], sc_vertices_expected[_i * 4 + 1], TEST_EPS);
  ck_assert_float_eq_tol(data[_i * 4 + 2], sc_vertices_expected[_i * 4 + 2], TEST_EPS);
  ck_assert_float_eq_tol(data[_i * 4 + 3], 1.0, TEST_EPS);
}
END_TEST

START_TEST(test_simple_cube_faces_nonull)
{

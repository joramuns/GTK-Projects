...
typedef struct deposit_input {
    ...
    unsigned term; /**< Amount of days to be deposited, different types of input as monthes, quarters or years evaluates the number of days */
    unsigned freq_payment; /**< Frequency of payment, uses enum as argument */
    unsigned capitalization; /**< Selected type of deposit: 0 - without capitalization, 1 - with capitalization */
    double daily_income; /**< Daily profit */
    double total_profit; /**< Total amount of money earned by the end of contract*/
    double *account_movement; /**< An array from GUI that may store a positive or negative value depends on type of operation - withdrawal or replenishment */
} deposit_input;
...
typedef struct deposit_output {
    double total_profit; /**< Total amount of money earned by the end of contract */
    double deposit; /**< Final amount of money in account after all payouts and withdrawals */
    node *stack_of_payouts; /**< Stores a node list with periodical payouts by deposit, needs to be freed*/
} deposit_output;
